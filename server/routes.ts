...
import { insertOrderSchema, insertContactSchema, CartItem, insertProductSchema, insertBlogPostSchema, insertCategorySchema, insertProductImageSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/key", adminAuth, async (req: Request, res: Response) => {
    ...
    if (!newKey || typeof newKey !== 'string' || newKey.length < 6) {
      return res.status(400).json({ message: "New key must be at least 6 characters" });
    }
  ...
  app.post("/api/admin/categories", adminAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.log("[express] Category created:", category);
